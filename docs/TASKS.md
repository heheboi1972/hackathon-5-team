...
| 3-6b | (2026-08-27 추가) 챗봇 실사용 중 발견된 3개 버그 수정 — ① "가장 많이 쓴 단어" 질문이 무응답이던 것을 새 `top_term` intent(regex 선분기, LLM 0회) + `top_terms` 툴로 지원 ② "이번주"/"저번에" 같은 상대 날짜 표현이 그냥 검색 텍스트로만 취급돼 못 찾던 것을 `_effective_range()`로 파싱해 fact/metric/report_query/term_count 공통 보강 ③ fact_query가 감정(결과)을 원인처럼 답하는 문제 — chat_answer.md에 원인/결과 구분 규칙 추가. 테스트 20+14개 통과(`test_chat_supervisor.py`/`test_term_search.py`) | 윤아 | — | 3-6 |
| 3-6c | (2026-08-27 추가) advice_request 안내 문구를 고정 문자열 → LLM 생성으로 전환 (윤아 요청 — "연락 줄었는데 어떡하지"류 리다이렉트 자체는 설계대로였지만, 매번 똑같은 문구 대신 LLM이 질문 맥락에 맞게 쓰게 해달라는 것). **감지(어떤 메시지가 advice_request인지)는 여전히 LLM 0회**(regex/chat_intent 그대로) — 바뀐 건 감지 이후 문구 생성뿐(`chat_answer_agent.py`에서 LLM 1회). 안전장치: `banned_patterns.txt`(safety_agent.py와 동일 목록)로 생성된 문구를 한 번 더 스캔해서 조언/판단/명령 표현이 섞이면 무조건 기존 고정 문구(`ADVICE_FALLBACK_TEXT`)로 폴백 — "관계를 판단하지 않는다"는 원칙을 LLM 혼자에게 맡기지 않음. citations도 항상 강제로 비움. 신규 테스트 7개(`test_agents.py` 4개 + `test_chat_supervisor.py` 3개, 기존 2개는 새 동작에 맞게 갱신) 포함 전체 스위트 199 passed / 3 skipped (기존에 알려진 무관한 mock-drift 실패 1건 제외). **실 watsonx 검증은 아직 안 됨** — mock/가짜 LLM으로만 배관·안전장치 로직을 확인했고, 실제 gpt-oss가 자연스러운 문구를 만드는지·banned_patterns 폴백이 얼마나 자주 걸리는지는 `scripts/3-6c_advice_agent_real_llm_test.py`(신규, 실행은 윤아 예정)로 본인 PC에서 확인 필요 | 윤아 | — | 3-6b |
