...
| P-3 | **양쪽 동의가 구조.** 상호 수락 후 활성화. 리포트는 양쪽 동일 공개. 예외: 자기 성찰 섹션("내 단어")은 본인에게만 표시하며, 챗봇 단어 횟수는 커플 합산만 제공(발화자별 미계산) |
...
- `metrics`: 추이형 3개 + 기준선 + delta + comparable
- `highlights`: 변화 1~3개. `comparable` 주차만. 각각 관찰 + **해석 ≥2개** + 근거 메시지 + 출처
