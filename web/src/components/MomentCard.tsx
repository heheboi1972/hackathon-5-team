...
export default function MomentCard({ moment }: { moment: Moment }) {
  ...
  const messages = moment.messages?.length
    ? moment.messages
    : [{ at: moment.at, text: moment.text }];

  return (
    <details className="report-moment-detail">
      <summary className="report-moment-detail__summary">
        <span className="report-moment-detail__summary-label">{moment.text}</span>
        <span className="report-moment-detail__toggle" aria-hidden="true">{hasMessages ? "메시지 보기" : "상세 보기"} <span>⌄</span></span>
      </summary>
      <div className="report-moment-detail__messages" aria-label="기억하고 싶은 순간의 메시지">
        <p className="report-moment-detail__eyebrow">{hasMessages ? "이 순간의 대화" : "이 순간의 기록"}</p>
        <div className="report-moment-detail__message-list">
          {messages.map((message, index) => (
            <blockquote className="report-moment-detail__message" key={`${message.at}-${index}`}>
